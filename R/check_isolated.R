#' Check code in an isolated R session.

#' @name check_isolated
#' @param test  [call]
#' @param package  [choice]  NULL is ok.  Defaults to current_pkg()
#' @return \code{run_isolated}: TRUE if the run generated no error, else an error message of class character.
#' @examples
...
#'  })
#' @export
check_isolated <- function(test, package = current_pkg()) {
    ...
    if (isTRUE(outs$result))
        return(TRUE)
    outs$error$message
    # Returns: TRUE if the run generated no error, else an error message of class character.
}
