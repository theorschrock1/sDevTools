...
build_snapshot_test <- function(fn_name, code, overwrite = FALSE, commit_git  = TRUE, push_github = TRUE) {
    ...
    find <- unique(expr_extract_call(code, fn_name))
    if (len0(find)) {
        g_stop("function '{fn_name}' not found in code")
    }
    replacement <- lapply(find, function(x) expr(expect_snapshot(!!x, cran = TRUE)))
    test_code <- expr_find_replace_all(find, replacement, code, match.first = FALSE)
    test_code = expr({
    ...
    if (is_dir_using_git() & commit_git) {
        add2Git(file = c(testpath, snaps), message = glue('added tests for "{fn_name}"'
), push = push_github,
            bump_version = FALSE)
    }
